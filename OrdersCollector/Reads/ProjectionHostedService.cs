...
namespace OrdersCollector.Reads
{
    // ReSharper disable once ClassNeverInstantiated.Global
    public class ProjectionHostedService : IHostedService
    {
        private static readonly ILogger Logger = Log.ForContext<ProjectionHostedService>();
        
        private readonly IDocumentStore _store;
        private IDaemon _daemon;

        public ProjectionHostedService(IDocumentStore store)
        {
            _store = store;
        }
        ...
        public Task WaitForNonStaleResults() => _daemon.WaitForNonStaleResults();

        private class SerilogDaemonLogger : IDaemonLogger
        {
            private static readonly ILogger Logger = Log.ForContext(Constants.SourceContextPropertyName, "MartenDeamon");

            public IEnumerable<string> ToNames(IEnumerable<IProjectionTrack> projectionTracks) =>
                projectionTracks.Select(ToName);

            public string ToName(IProjectionTrack track) => track.ViewType.Name;

            public object ToPageSummary(EventPage page) => new {page.From, page.To, page.Count};
            ...
            {
                Logger.Information("Finished starting async daemon");
            }

            public void BeginRebuildAll(IEnumerable<IProjectionTrack> values)
            {
                Logger.Information("Beggining a rebuild of {@Projections}", ToNames(values));
            }
            ...
            {
                Logger.Information("Beggining to stop async daemon");
            }

            public void AllStopped()
            {
                Logger.Information("Async daemon stopped");
            }
            ...
            {
                Logger.Error(exception, "Error in async daemon");
            }
        }
    }
}